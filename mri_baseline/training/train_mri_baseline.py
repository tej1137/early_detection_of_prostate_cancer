...
from mri_baseline.models.mri_encoder import MRIEncoder, MRIClassifier
...
def build_dataloaders(train_cfg: TrainConfig, data_cfg: DataConfig):
    data_cfg.augment_train = train_cfg.augment_train

    train_ds = PiCAIDataset("train", data_cfg, augment=train_cfg.augment_train, mri_only=True)
    val_ds   = PiCAIDataset("val",   data_cfg, augment=False, mri_only=True)
    test_ds  = PiCAIDataset("test",  data_cfg, augment=False, mri_only=True)
...
def save_training_curves(history, run_dir):
    ...
    plt.tight_layout()
    plt.savefig(run_dir / "mri_training_curves.png", dpi=150)
    plt.close()
...
def save_confusion_matrix(labels, preds, run_dir):
    ...
    plt.title("MRI Baseline — Confusion Matrix")
    plt.ylabel("True"); plt.xlabel("Predicted")
    plt.tight_layout()
    plt.savefig(run_dir / "mri_confusion_matrix.png", dpi=150)
    plt.close()
...
def main():
    ...
    best_auroc = 0.0
    no_improve = 0
    best_ckpt  = run_dir / "best_mri_model.pt"
    ...
    with open(run_dir / "mri_results.json", 'w') as f:
