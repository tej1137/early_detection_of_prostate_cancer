...
def build_dataloaders(train_cfg: TrainConfig, data_cfg: DataConfig):
    train_ds = PiCAIDataset("train", data_cfg, augment=train_cfg.augment_train)
...
def save_training_curves(history, run_dir):
    ...
    plt.tight_layout()
    plt.savefig(run_dir / "fusion_training_curves.png", dpi=150)
    plt.close()
...
def save_confusion_matrix(labels, preds, run_dir):
    ...
    plt.tight_layout()
    plt.savefig(run_dir / "fusion_confusion_matrix.png", dpi=150)
    plt.close()
...
def main():
    ...
    print("=" * 60)
    print("FUSION MODEL TRAINING  (MRI + PSA)")
    ...
    best_auroc = 0.0
    no_improve = 0
    best_ckpt  = run_dir / "best_fusion_model.pt"
    ...
    with open(run_dir / "fusion_results.json", 'w') as f:
